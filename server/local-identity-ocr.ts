...
async function getWorker() {
  if (!workerPromise) {
    const cachePath = process.env.IDENTITY_OCR_CACHE_PATH?.trim()
...
function inferFields(rawText: string, documentType: IdentityDocumentType) {
  ...
  const dateOfBirth = pickField(rawText, ['الميلاد', 'الولادة', 'birth', 'dob'])
    || (text.match(/(?:19|20)\d{2}[\/.\-]\d{1,2}[\/.\-]\d{1,2}/)?.[0] || null)
...
export async function analyzeIdentityDocumentLocally(input: {
  ...
  try {
    ...
    if (!hasAnyField) {
      return {
        status: 'NO_RESULT', reason: 'NO_DOCUMENT_RESULT', provider: 'محرك OCR محلي', confidence: null,
        fields: { fullName: null, documentNumber: null, dateOfBirth: null, nationality: null, sex: null, expiryDate: null },
        documentTypeDetected: input.documentType, faceCrop: null,
    ...
    return {
      status: 'COMPLETED', reason: 'COMPLETED', provider: 'محرك OCR محلي', confidence,
      fields: {
        fullName: fields.fullName, documentNumber: fields.documentNumber, dateOfBirth: fields.dateOfBirth,
        nationality: fields.nationality, sex: fields.sex, expiryDate: fields.expiryDate,
      },
    ...
    workerPromise = null
    return {
      status: 'PROVIDER_UNAVAILABLE', reason: 'PROVIDER_UNAVAILABLE', provider: 'محرك OCR محلي', confidence: null,
      fields: { fullName: null, documentNumber: null, dateOfBirth: null, nationality: null, sex: null, expiryDate: null },
      documentTypeDetected: null, faceCrop: null,
      faceComparison: { status: 'MANUAL_REVIEW_REQUIRED', confidence: null },
    }
  }
}
