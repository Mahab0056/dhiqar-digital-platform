...
const isAllowedRealtimeOrigin = (origin?: string) =>
...
app.get('/api/citizen/feedback/:reference/media/:mediaId', requireSession('CITIZEN'), (req, res) => {
  ...
  const linked = db
    ...
    .get(req.params.reference, citizen.id, req.params.mediaId)
...
app.patch('/api/employee/service-requests/:reference', requireSession('EMPLOYEE', 'SUPER_ADMIN'), async (req, res) => {
  const row = db
    ...
    .get(req.params.reference) as Record<string, unknown> | undefined
  ...
  db.prepare(
    ...
    parsed.data.status === 'ACTION_REQUIRED' ? parsed.data.requiredDocument : null,
    timestamp,
  ...
  const updated = db
    ...
    .get(req.params.reference) as Record<string, unknown>
  ...
  (req, res) => {
    try {
      ...
      WHERE sr.reference = ? AND sr.citizen_id = ?`
        ...
        .get(req.params.reference, citizen.id) as Record<string, unknown> | undefined
  ...
  async (req, res) => {
    try {
      ...
      if (screening.qualityStatus === 'NEEDS_RECAPTURE')
        return res.status(422).json({ message: 'فحص الجودة الآلي طلب إعادة التصوير قبل حفظ بيانات الهوية.', screening })
      ...
      const extractionSummary = JSON.stringify({
        ...
        documentTypeDetected: analysis.documentTypeDetected,
        fields: analysis.fields,
        fieldsPresent: Object.fromEntries(Object.entries(analysis.fields).map(([key, value]) => [key, Boolean(value)])),
    ...
    `
        ...
        locationAllowed ? payload.locationLat : null,
        locationAllowed ? payload.locationLng : null,
  ...
  (req, res) => {
    try {
      const payload = z
        ...
        .parse(req.body)
      const review = db.prepare('SELECT * FROM identity_reviews WHERE id = ?').get(req.params.id) as
...
app.post(
  ...
  `
    ...
    if (propertyDocument)
      protectedFiles.push({ file: propertyDocument, purpose: 'APPLICATION_DOCUMENT', label: requiredPropertyDocument })
    if (storefrontPhoto)
    ...
    addAudit({
      ...
      metadata: { protectedAttachments: protectedFiles.map(file => file.label), retentionDays: 30, faceConsent: true },
    })
...
app.post('/api/applications/:reference/request-document', requireSession('EMPLOYEE', 'SUPER_ADMIN'), (req, res) => {
  ...
  notifyCitizen({
    ...
    message: `ارفع ${payload.documentName} لإكمال تدقيق المعاملة ${req.params.reference}.`,
    link: `/citizen/application/${req.params.reference}`,
  })
  addAudit({
    actor: 'سارة كاظم حسن',
    role: 'EMPLOYEE',
...
app.post('/api/applications/:reference/approve', requireSession('EMPLOYEE', 'SUPER_ADMIN'), async (req, res) => {
  ...
  if ((item.fee as number) > 0) {
    ...
    notifyCitizen({
      ...
      message: `تمت الموافقة الإدارية على ${req.params.reference}. سيُفتح الدفع عند ربط بوابة الدفع المعتمدة.`,
      link: `/citizen/application/${req.params.reference}`,
    })
    addAudit({
      actor: 'سارة كاظم حسن',
...
app.get('/api/verify/:verificationId', (req, res) => {
  ...
  if (issued && issued.status === 'ACTIVE') {
    ...
    return res.json({
      ...
      originalPdfUrl: `/api/verify/${req.params.verificationId}/original-pdf`,
      pdfAvailable: true,
    })
  }
  ...
  addAudit({
    ...
    entityType: 'Document',
    entityId: req.params.verificationId,
...
app.get('/api/platform-services/:key', (req, res) => {
  const item = db
    ...
    .get(req.params.key) as Record<string, unknown> | undefined
...
app.get('/api/super-admin/department-workbench', requireSession('SUPER_ADMIN'), (_req, res) => {
  ...
  const normalisedApplications = applicationRows.map(item => ({
    ...item,
...
app.post(
  ...
  (req, res) => {
    ...
    addAudit({
      ...
      entityId: id,
      metadata: { departmentId: payload.departmentId, observedAt: payload.observedAt, sourceName: payload.sourceName },
    })
