...
describe('staff accounts & sessions', () => {
  ...
  it('locks the account after repeated failures', async () => {
    for (let i = 0; i < 5; i++) await request(app).post('/api/auth/staff/login').send({ username: 'rev.one', password: 'nope' })
    const locked = await request(app).post('/api/auth/staff/login').send({ username: 'rev.one', password: 'Rotated-rev.one-2026!' })
  ...
  it('disabling an account kills its sessions', async () => {
    ...
    expect((await request(app).get('/api/applications').set('Cookie', cookie)).status).toBe(401)
    const login = await request(app).post('/api/auth/staff/login').send({ username: 'emp.two', password: 'Rotated-emp.two-2026!' })
  ...
  it('cannot disable the last super admin', async () => {
    ...
    const response = await request(app).post(`/api/super-admin/staff/${me.id}/status`).set('Cookie', admin).send({ status: 'DISABLED' })
...
describe('MFA (TOTP)', () => {
  it('enrolls, requires code on login, blocks replay', async () => {
    ...
    const confirm = await request(app).post('/api/auth/staff/mfa/confirm').set('Cookie', cookie).send({ code: totpCode(secret) })
    expect(confirm.status).toBe(200)

    const login = await request(app).post('/api/auth/staff/login').send({ username: 'ops.one', password: 'Rotated-ops.one-2026!' })
    expect(login.body.mfaRequired).toBe(true)
    // same time-step code was consumed during enrollment -> replay rejected
    const replay = await request(app).post('/api/auth/staff/mfa').send({ challengeToken: login.body.challengeToken, code: totpCode(secret) })
