...
## 3ب. الحسابات والصلاحيات (منفذة)

| العنصر | التنفيذ |
|---|---|
| الحسابات | جدول `staff_accounts`: اسم مستخدم، اسم كامل، دور، دائرة، scrypt hash، حالة، قفل |
| الأدوار | `EMPLOYEE`, `IDENTITY_REVIEWER`, `OPERATIONS`, `SUPER_ADMIN` + `CITIZEN` |
| MFA | TOTP (RFC 6238) بسر مشفر AES-256-GCM، حماية من إعادة الاستخدام |
| الجلسات | جدول `auth_sessions`: الكوكي يحمل معرّف موقّع فقط؛ إلغاء فوري، مهلة خمول ساعة، انتهاء 12 ساعة للموظف |
| القفل | 5 محاولات فاشلة → قفل 15 دقيقة |
| التدقيق | كل إجراء يُسجل بـ`الاسم (username)` الفعلي |
| نطاق الدائرة | لوحة `/department/:id` متاحة لموظفي الدائرة نفسها، غرفة العمليات، والمدير العام |
...
| اختيار IAM وOTP وIdentity Provider | فورية (حسابات + MFA منفذة داخلياً؛ ربط IAM حكومي لاحقاً) |
| تحويل SQLite إلى PostgreSQL (انظر docs/DATABASE.md) | عالية — بعد تثبيت نطاق الـpilot |
