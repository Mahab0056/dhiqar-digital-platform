...
const payload = Buffer.from(
  JSON.stringify({ sub: String(citizenId), role: 'CITIZEN', exp: Math.floor(Date.now() / 1000) + 3600 })
).toString('base64url')
...
if (missingResponse.status !== 400 || !missingBody.message?.includes('عقد الإيجار'))
  throw new Error(`missing file reason not clear: ${missingResponse.status} ${missingBody.message}`)
...
invalid.append(
  'faceVideo',
  new Blob([Buffer.concat([Buffer.from([0x1a, 0x45, 0xdf, 0xa3]), Buffer.alloc(110_000)])], { type: 'video/webm' }),
  'face-video.webm'
)
...
if (
  ...
  !invalidBody.message?.includes('لا يطابق صيغة آمنة') ||
  !invalidBody.message.includes('أعد تصويره')
)
  throw new Error(`invalid file reason not actionable: ${invalidResponse.status} ${invalidBody.message}`)
