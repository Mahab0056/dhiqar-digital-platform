...
const row = db.prepare("SELECT verification_id FROM issued_documents ORDER BY issued_at DESC LIMIT 1").get() as { verification_id?: string } | undefined
...
if (verificationResponse.status !== 200 || !verification.originalPdfUrl) throw new Error(`Verification lookup failed: ${verificationResponse.status}`)
const response = await fetch(`${base}${verification.originalPdfUrl}`)
const bytes = Buffer.from(await response.arrayBuffer())
if (response.status !== 200 || !bytes.subarray(0, 4).equals(Buffer.from('%PDF'))) throw new Error(`PDF export failed: ${response.status}`)
