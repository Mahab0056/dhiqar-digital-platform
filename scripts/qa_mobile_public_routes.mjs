...
const evaluate = async expression => { const result = await send('Runtime.evaluate', { expression, returnByValue: true, awaitPromise: true }); if (result.exceptionDetails) throw new Error(result.exceptionDetails.text || 'فشل فحص الهاتف.'); return result.result.value }
const delay = milliseconds => new Promise(resolve => setTimeout(resolve, milliseconds))
const errors = []
socket.addEventListener('message', event => { const message = JSON.parse(event.data); if (message.method === 'Runtime.exceptionThrown') errors.push(message.params.exceptionDetails.text || 'استثناء تشغيل') })
...
const failed = results.filter(item => item.path !== item.expectedPath || item.notFound || !item.text.includes(item.expected) || item.horizontalOverflow > 1 || item.controls < 1)
