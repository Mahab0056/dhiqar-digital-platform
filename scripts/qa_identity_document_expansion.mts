...
const review = await submission.json() as { id: string; documentType: string; files: Array<{ id: string; retentionPolicy: string }> }
if (review.documentType !== 'PASSPORT' || review.files.length !== 2 || review.files.some(file => file.retentionPolicy !== 'RETAINED_WITH_CONSENT')) throw new Error('passport flow did not retain expected media')
...
const approved = await fetch(`${base}/api/admin/identity-reviews/${review.id}/decision`, { method: 'POST', headers: superHeaders, body: JSON.stringify({ decision: 'APPROVED', notes: 'اختبار قاعدة معزولة' }) })
...
purgeExpiredMedia()
const mediaRows = db.prepare('SELECT storage_path, retention_policy, deleted_at FROM media_objects WHERE citizen_id = ? ORDER BY created_at DESC LIMIT 2').all(citizenId) as Array<{ storage_path: string; retention_policy: string; deleted_at: string | null }>
