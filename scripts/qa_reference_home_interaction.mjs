...
const categories = await evaluate(
  `(() => [...document.querySelectorAll('.civic-search select option')].map(option => option.value))()`
)
...
const destination = await evaluate(
  `(() => ({ path: location.pathname, heading: document.querySelector('main h1')?.textContent?.trim() || '' }))()`
)
