...
const reviewerHeaders = { cookie: `dhiqar_session=${sign('super-admin', 'SUPER_ADMIN')}`, 'content-type': 'application/json' }
...
const submission = await fetch(`${base}/api/onboarding/identity-review`, { method: 'POST', headers: citizenHeaders, body: form })
if (submission.status !== 201) throw new Error(`identity review status ${submission.status}: ${await submission.text()}`)
const created = await submission.json() as { id: string; analysis: { fields: { fullName: string | null; documentNumber: string | null; dateOfBirth: string | null; nationality: string | null; sex: string | null; expiryDate: string | null }; faceComparison: { status: string; confidence: number | null } } }
if (process.env.EXPECT_FACE_MATCH === 'true' && (created.analysis.faceComparison.status !== 'MATCH_ASSISTED' || created.analysis.faceComparison.confidence === null)) throw new Error('provider face comparison was not returned to citizen submission')
...
if ('extractedFields' in citizen || 'dateOfBirth' in citizen || 'expiryDate' in citizen) throw new Error('protected OCR fields leaked to citizen endpoint')
...
const queue = await queueResponse.json() as Array<{ id: string; extractedFields: { documentTypeDetected: string | null; fullName: string | null; documentNumber: string | null; dateOfBirth: string | null; nationality: string | null; sex: string | null; expiryDate: string | null } | null; screening: { faceMatchStatus: string; faceMatchScore: number | null } }>
...
if (process.env.EXPECT_FACE_MATCH === 'true' && (review.screening.faceMatchStatus !== 'MATCH_ASSISTED' || review.screening.faceMatchScore === null)) throw new Error('provider face comparison was not retained for reviewer')
for (const key of ['fullName', 'documentNumber', 'dateOfBirth', 'nationality', 'sex', 'expiryDate'] as const) {
  if (review.extractedFields[key] !== created.analysis.fields[key]) throw new Error(`reviewer OCR field mismatch: ${key}`)
}
