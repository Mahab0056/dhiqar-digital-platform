...
if (!preflight.faceSection || !preflight.cameraButton.includes('فتح الكاميرا'))
  throw new Error(`تعذر تجهيز واجهة فيديو الوجه: ${JSON.stringify(preflight)}`)
await evaluate(
  `(() => { document.querySelector('.service-face-confirmation .capture-actions .button.secondary')?.click(); return true })()`
)
await delay(2400)
...
if (
  !liveCapture.liveCamera ||
  !liveCapture.frame ||
  !liveCapture.countdownActive ||
  liveCapture.countdown !== '5' ||
  liveCapture.instruction !== 'ابتسم للكاميرا' ||
  !liveCapture.detail.includes('ابتسامة') ||
  liveCapture.progress === '0%' ||
  !liveCapture.cameraBounds ||
  !liveCapture.frameBounds ||
  liveCapture.frameBounds.top < liveCapture.cameraBounds.top ||
  liveCapture.frameBounds.right > liveCapture.cameraBounds.right ||
  liveCapture.frameBounds.bottom > liveCapture.cameraBounds.bottom ||
  liveCapture.frameBounds.left < liveCapture.cameraBounds.left
) {
...
console.log(
  JSON.stringify(
    { face_capture_guidance: 'pass', preflight, liveCapture, screenshot: '/tmp/dhiqar-face-capture-mobile.png' },
    null,
    2
  )
)
