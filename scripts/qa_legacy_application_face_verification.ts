...
const blocked = await fetch(`${base}/api/applications/${reference}/approve`, {
  method: 'POST',
  headers: employeeHeaders,
})
if (blocked.status !== 409 || !(await blocked.text()).includes('فيديو توثيق الوجه'))
  throw new Error('legacy application approval was not safely blocked with a clear reason')
...
if (requestBody.status !== 'ACTION_REQUIRED' || requestBody.requiredDocument !== 'فيديو توثيق الوجه القصير')
  throw new Error('legacy application was not routed to face verification completion')
...
const uploaded = await fetch(`${base}/api/applications/${reference}/upload-document`, {
  method: 'POST',
...
const uploadedBody = (await uploaded.json()) as {
  status: string
  attachments: Array<{ purpose?: string; label: string; available: boolean }>
}
if (
  uploadedBody.status !== 'UNDER_REVIEW' ||
  !uploadedBody.attachments.some(file => file.purpose === 'FACE_VIDEO' && file.available)
)
  throw new Error('face video was not stored as protected application media')
const approved = await fetch(`${base}/api/applications/${reference}/approve`, {
  method: 'POST',
  headers: employeeHeaders,
})
if (approved.status !== 200)
  throw new Error(`verified legacy application approval status ${approved.status}: ${await approved.text()}`)
const approvedBody = (await approved.json()) as {
  status: string
  verificationId: string | null
  documentNumber: string | null
}
if (approvedBody.status !== 'APPROVED' || !approvedBody.verificationId || !approvedBody.documentNumber)
  throw new Error('verified legacy application did not issue document')
