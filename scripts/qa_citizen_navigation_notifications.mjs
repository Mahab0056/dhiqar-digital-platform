...
const fixture = { unread: 1, items: [{ id: 'qa-notification-1', type: 'SERVICE_REQUEST_CREATED', title: 'تم تسجيل طلب الفحص', message: 'وصل الطلب إلى الدائرة المختصة.', link: '/citizen', readAt: null, createdAt: '2026-08-28T12:00:00.000Z' }] }
...
if (!portalLoaded.hasSidebar || !portalLoaded.hasMenuButton || portalLoaded.notificationHref !== '/citizen/notifications' || !portalLoaded.navLinks.includes('/citizen/notifications')) {
  throw new Error(`قائمة المواطن أو رابط الإشعارات غير مكتمل: ${JSON.stringify(portalLoaded)}`)
}

await evaluate(`(() => { document.querySelector('.mobile-sidebar-button')?.click(); return document.querySelector('.portal-sidebar')?.classList.contains('open') })()`)
...
await evaluate(`(() => { document.querySelector('.portal-sidebar nav a[href="/citizen#services"]')?.click(); return true })()`)
await delay(700)
const serviceNavigation = await evaluate(`(() => ({ path: location.pathname, hash: location.hash, sidebarOpen: document.querySelector('.portal-sidebar')?.classList.contains('open') || false, servicesPresent: Boolean(document.getElementById('services')) }))()`)
if (serviceNavigation.path !== '/citizen' || serviceNavigation.hash !== '#services' || serviceNavigation.sidebarOpen || !serviceNavigation.servicesPresent) {
...
if (notificationsView.path !== '/citizen/notifications' || notificationsView.heading !== 'إشعارات الحساب' || notificationsView.rowCount !== 1 || !notificationsView.markAllVisible || !notificationsView.unreadRow) {
...
await delay(180)
const marked = await evaluate(`(() => ({ markAllVisible: Boolean(document.querySelector('.citizen-notifications-head button')), unreadRows: document.querySelectorAll('.citizen-notifications-full-list .unread').length, readRows: document.querySelectorAll('.citizen-notifications-full-list .read').length }))()`)
if (marked.markAllVisible || marked.unreadRows !== 0 || marked.readRows !== 1) throw new Error(`تعذر تعليم الإشعارات كمقروءة: ${JSON.stringify(marked)}`)

const screenshot = await send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: true })
await writeFile('/tmp/dhiqar-citizen-notifications-mobile.png', screenshot.data, 'base64')
console.log(JSON.stringify({ citizen_navigation_notifications: 'pass', portalLoaded, serviceNavigation, notificationsView, marked, screenshot: '/tmp/dhiqar-citizen-notifications-mobile.png' }, null, 2))
socket.close()
