...
serviceForm.append(
  ...
  JSON.stringify({
    ...
    propertyNumber: 'QA-101',
    propertyAddress: 'عنوان اختبار صناعي',
    district: 'الناصرية',
    constructionType: 'سكني',
    floors: '1',
    engineerName: 'مكتب اختبار',
  })
)
...
if (createdResponse.status !== 201)
  throw new Error(`service creation failed: ${createdResponse.status} ${await createdResponse.text()}`)
...
const approvalResponse = await fetch(`${base}/api/employee/service-requests/${created.reference}`, {
  ...
  body: JSON.stringify({
    status: 'APPROVED',
    currentAction: 'اكتملت مراجعة البيانات واعتمدت المعاملة ضمن اختبار الأرشفة.',
  }),
})
if (approvalResponse.status !== 200)
  throw new Error(`approval failed: ${approvalResponse.status} ${await approvalResponse.text()}`)
...
const documents = (await archiveResponse.json()) as Array<{
...
if (
  protectedPdf.status !== 200 ||
  !protectedPdfBytes.subarray(0, 4).equals(Buffer.from('%PDF')) ||
  !protectedPdf.headers.get('content-disposition')?.startsWith('inline')
)
  throw new Error(`citizen original PDF failed: ${protectedPdf.status}`)
...
if (
  ...
  !downloadedPdfBytes.subarray(0, 4).equals(Buffer.from('%PDF')) ||
  !downloadedPdf.headers.get('content-disposition')?.startsWith('attachment')
)
  throw new Error(`citizen PDF download failed: ${downloadedPdf.status}`)
...
if (publicVerify.status !== 200 || !verification.pdfAvailable || !verification.originalPdfUrl)
  throw new Error(`public verification failed: ${publicVerify.status}`)
const publicPdf = await fetch(`${base}${verification.originalPdfUrl}`)
const publicPdfBytes = Buffer.from(await publicPdf.arrayBuffer())
if (publicPdf.status !== 200 || !publicPdfBytes.subarray(0, 4).equals(Buffer.from('%PDF')))
  throw new Error(`public original PDF failed: ${publicPdf.status}`)
...
appForm.append('serviceKey', 'store-license')
appForm.append('serviceName', 'إجازة فتح محل')
appForm.append('department', 'مديرية بلديات ذي قار')
appForm.append('businessName', 'محل اختبار أرشفة')
appForm.append('activityType', 'متجر')
appForm.append('address', 'عنوان اختبار صناعي')
appForm.append('district', 'الناصرية')
appForm.append('ownershipType', 'rent')
appForm.append('coordinates', JSON.stringify({ lat: 31.05, lng: 46.26 }))
appForm.append('fee', '0')
appForm.append('faceConsent', 'true')
...
const appApproval = await fetch(`${base}/api/applications/${application.reference}/approve`, {
  method: 'POST',
  headers: { cookie: employeeCookie },
})
if (appApproval.status !== 200)
  throw new Error(`application approval failed: ${appApproval.status} ${await appApproval.text()}`)
const archiveAfterApplication = await fetch(`${base}/api/citizen/issued-documents`, {
  headers: { cookie: citizenCookie },
})
const documentsAfterApplication = (await archiveAfterApplication.json()) as Array<{
...
if (
  applicationPdf.status !== 200 ||
  !Buffer.from(await applicationPdf.arrayBuffer())
    .subarray(0, 4)
    .equals(Buffer.from('%PDF')) ||
  !applicationPdf.headers.get('content-disposition')?.startsWith('inline')
)
  throw new Error('application archived PDF could not be opened')
const applicationDownload = await fetch(`${base}${applicationDocument.pdfDownloadUrl}`, {
  headers: { cookie: citizenCookie },
})
if (
  applicationDownload.status !== 200 ||
  !applicationDownload.headers.get('content-disposition')?.startsWith('attachment')
)
  throw new Error('application archived PDF could not be downloaded')
const unauthorizedArchive = await fetch(`${base}/api/citizen/issued-documents`)
if (unauthorizedArchive.status !== 401)
  throw new Error(`citizen archive unexpectedly public: ${unauthorizedArchive.status}`)
