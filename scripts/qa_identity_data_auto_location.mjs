...
const evaluate = async expression => {
  ...
  if (response.exceptionDetails) throw new Error(response.exceptionDetails.text || 'تعذر فحص الهوية.')
...
await delay(700)
await evaluate(
  `(() => { document.querySelector('.form-stage .stage-actions .button.primary')?.click(); return true })()`
)
await delay(300)
const backAttached = await attachImage()
if (!backAttached) throw new Error('تعذر إدخال ظهر المستند في الفحص.')
await delay(250)
const reviewStep = await evaluate(
  `(() => ({ text: document.querySelector('.form-stage')?.innerText || '', locationButton: Array.from(document.querySelectorAll('button')).some(button => button.textContent?.includes('تحديد موقعي')), fields: Array.from(document.querySelectorAll('.identity-document-data-grid strong')).map(item => item.textContent?.trim()) }))()`
)
...
if (reviewStep.locationButton || !requiredValues.every(value => reviewStep.text.includes(value)))
  throw new Error(`عرض بيانات أو موقع غير صحيح: ${JSON.stringify(reviewStep)}`)
...
const continuation = await evaluate(
  `(() => ({ heading: document.querySelector('.form-stage h2')?.textContent?.trim() || '', geoCalls: window.__geoCalls, hasFaceCapture: Boolean(document.querySelector('.secure-capture .capture-head')) }))()`
)
if (continuation.geoCalls !== 1 || !continuation.heading.includes('تأكيد الوجه') || !continuation.hasFaceCapture)
  throw new Error(`لم يعمل GPS التلقائي أو الانتقال للوجه: ${JSON.stringify(continuation)}`)
console.log(
  JSON.stringify(
    {
      identity_data_auto_location: 'pass',
      reviewStep,
      continuation,
      screenshot: '/tmp/dhiqar-identity-review-data-mobile.png',
    },
    null,
    2
  )
