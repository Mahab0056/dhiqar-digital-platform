...
const verify = await call('citizen', '/api/onboarding/verify-phone', {
  method: 'POST',
  body: { phone, challengeId: otp.data.challengeId, otp: '246810' },
})
...
await call('reviewer', '/api/auth/staff/login', {
  method: 'POST',
  body: { username: 'reviewer.qa', password: 'Audit-Pass-2026!' },
})
...
if (pending) {
  const decision = await call('reviewer', `/api/admin/identity-reviews/${pending.id}/decision`, {
    method: 'POST',
    body: { decision: 'APPROVED', notes: 'مطابقة مؤكدة يدوياً' },
  })
...
await call('employee', '/api/auth/staff/login', {
  method: 'POST',
  body: { username: 'emp.muni', password: 'Emp-Muni-2026!' },
})
...
const service = (services.data.items || []).find(
  s =>
    s.departmentId === 'dhiqar-municipalities' &&
    s.mode === 'CATALOG' &&
    s.requiredDocuments.length > 0 &&
    s.channel === 'ONLINE_SUBMISSION'
)
...
if (service) {
  ...
  for (const field of service.fields || []) {
    data[field.key] = field.options?.length
      ? field.options[0]
      : field.type === 'tel'
        ? '07801234567'
        : field.type === 'email'
  ...
  for (const doc of service.requiredDocuments)
    form.set(`doc__${doc.key}`, new Blob([jpeg], { type: 'image/jpeg' }), `${doc.key}.jpg`)
...
if (reference) {
  ...
  check(
    ...
    (list.data.items || []).every(item => item.departmentId === 'dhiqar-municipalities')
  )
  const fee = await call('employee', `/api/employee/service-requests/${reference}`, {
    method: 'PATCH',
    body: { status: 'PAYMENT_REQUIRED', currentAction: 'رسم الخدمة المقرر من الدائرة', amountIqd: 5000 },
  })
  ...
  if (payRef) {
    ...
    check(
      'unauthenticated sandbox settlement refused',
      (openPay.headers.get('location') || '').includes('invalid'),
      openPay.headers.get('location') || ''
    )
    const paid = await call('citizen', `/api/citizen/payments/${payRef}/sandbox-confirm`, {
      method: 'POST',
      body: { outcome: 'PAID' },
    })
    check(
      'citizen pays in sandbox',
      paid.status === 200 && paid.data.status === 'PAID',
      paid.data?.status || String(paid.status)
    )
  }
  const after = await call('citizen', '/api/citizen/service-requests')
  const released = (after.data || []).find(r => r.reference === reference)
  check(
    'paid request returns to the department queue',
    ['SUBMITTED', 'UNDER_REVIEW'].includes(released?.status),
    released?.status
  )
  const detail = await call('employee', `/api/employee/service-requests/${reference}`)
  for (const item of detail.data.checklist || []) {
    if (item.mediaId)
      await call('employee', `/api/employee/service-requests/${reference}/documents/${item.key}`, {
        method: 'PATCH',
  ...
  const approve = await call('employee', `/api/employee/service-requests/${reference}`, {
    method: 'PATCH',
    body: { status: 'APPROVED', currentAction: 'اكتملت المعاملة وصدرت الوثيقة', decisionNote: 'مستوفٍ للشروط' },
  })
  ...
  if (issued) {
    const pdf = await fetch(`${base}/api/citizen/issued-documents/${issued.id}/pdf`, {
      headers: { cookie: jar.citizen },
    })
    const buffer = Buffer.from(await pdf.arrayBuffer())
    check(
      'PDF downloads',
      pdf.status === 200 && buffer.subarray(0, 4).toString() === '%PDF',
      `${pdf.status} ${buffer.length} bytes`
    )
    const pub = await call('public', `/api/verify/${issued.verificationId}`)
    check(
      'public verification returns a minimal record',
      pub.status === 200 && pub.data.status === 'APPROVED' && !('address' in pub.data) && !('citizenId' in pub.data),
      Object.keys(pub.data).join(',')
    )
  }
}
