...
for (const [id, d] of Object.entries(R))
  if (id.endsWith('-1440-light'))
    for (const l of d.links) hrefs.add(JSON.stringify([l.href, l.text, l.sel, id.replace('-1440-light', '')]))
const byHref = {}
for (const s of hrefs) {
  const [h, t, sel, pg] = JSON.parse(s)
  ;(byHref[h] ||= []).push(`${pg}:${sel}«${t}»`)
}
const results = []
for (const [h, where] of Object.entries(byHref)) {
  ...
  if (!h || h === '#' || h.startsWith('javascript')) status = 'DEAD(#/empty)'
    ...
    const info = await page.evaluate(
      hash => ({
        nf: /الصفحة غير موجودة/.test(document.body.innerText),
        url: location.pathname,
        anchor: hash ? !!document.getElementById(hash) : null,
