...
const watch = (page, bucket) => {
  ...
  page.on('console', m => {
    if (m.type() === 'error' && !/ERR_TUNNEL|openstreetmap|nominatim|401|404/.test(m.text()))
      bucket.push(`console: ${m.text().slice(0, 160)}`)
  })
}
...
if (!process.env.SKIP_HOME) {
  ...
  const chips = await page
    .locator('.gov-search ~ * button, .hero-suggestions button, [class*=suggest] button')
    .allTextContents()
    .catch(() => [])
  ...
  if (await eng.count()) {
    ...
    await page.waitForTimeout(500)
    note('after clicking English: url/dir/lang', {
      url: page.url(),
      dir: await page.evaluate(() => document.documentElement.dir),
      lang: await page.evaluate(() => document.documentElement.lang),
      sample: (await page.textContent('h1'))?.slice(0, 80),
    })
  }
...
if (!process.env.SKIP_ONB) {
  const ctx = await browser.newContext({
    ...
    geolocation: { latitude: 31.05, longitude: 46.25 },
  })
  const page = await ctx.newPage()
  await page.route('**/api/onboarding/identity-extract-preview', route =>
    route.fulfill({
      status: 200,
  ...
  if (!page.url().includes('/onboarding'))
    await page.goto(base + '/onboarding?continue=/service/health-birth-certificate', { waitUntil: 'networkidle' })
  ...
  note('onboarding after otp: url + step', {
    url: page.url(),
    stepper: await page
      .locator('.stepper')
      .innerText()
  ...
  note(
    'onboarding after upload: analysis status',
    await page.locator('.automatic-analysis-status, .form-notice, .form-success, .form-error').allTextContents()
  )
  ...
  await page.setInputFiles('.secure-capture input[type=file]', {
    name: 'back.jpg',
    mimeType: 'image/jpeg',
    buffer: jpeg,
  })
  ...
  await page.click('.stage-actions button.primary')
  await page.waitForTimeout(2500)
  note('onboarding step4 (after geolocation) heading/messages', {
    h2: await page.locator('.onboarding-panel h2').innerText(),
    msgs: await page.locator('.form-error, .form-success').allTextContents(),
  })
...
if (!process.env.SKIP_CIT) {
  ...
  if (!process.env.FROM_PAY) {
    ...
    await page.waitForTimeout(600)
    note('click تصفح الخدمات (hash link): url + scrollY', {
      url: page.url(),
      before,
    ...
    const pdfHref = await page
      .locator('#issued-documents a.button.primary')
      .first()
      .getAttribute('href')
      .catch(() => null)
    if (pdfHref) {
    ...
    note(
      ...
      (
        await page
          .locator('.citizen-notifications-full-list')
          .innerText()
          .catch(() => 'empty')
      ).slice(0, 1200)
    )
    ...
    await page.waitForTimeout(1500)
    note(
      'address search hits (external blocked)',
      await page
        .locator('.gov-location-hits')
        .innerText()
        .catch(() => 'no dropdown')
    )
    ...
    note(
      'support button attrs',
      await supportBtn.evaluate(b => ({ onclick: b.onclick, type: b.type, text: b.textContent }))
    )
    await page.goto(base + '/citizen/application/TQD-2026-0001', { waitUntil: 'networkidle' })
    await page.waitForTimeout(500)
    note(
      'approved-with-fee application text',
      (await page.locator('.current-action, .detail-aside').allInnerTexts()).join(' | ')
    )
    // payment flow
    await page.goto(base + '/citizen/pay/PAY-2026-00001', { waitUntil: 'networkidle' })
    await page.waitForTimeout(500)
  ...
  note(
    'sandbox buttons disabled after in-app navigation?',
    await page.locator('.payment-sandbox button').evaluateAll(bs => bs.map(b => b.disabled))
  )
  await page.reload({ waitUntil: 'networkidle' })
  await page.waitForTimeout(500)
  note(
    'sandbox buttons disabled after reload?',
    await page.locator('.payment-sandbox button').evaluateAll(bs => bs.map(b => b.disabled))
  )
  ...
  note(
    'mobile sidebar open geometry',
    await page.evaluate(() => {
      const r = document.querySelector('.portal-sidebar').getBoundingClientRect()
  ...
  note(
    'mobile topbar geometry',
    await page.evaluate(() => {
      const r = document.querySelector('.portal-topbar').getBoundingClientRect()
      return { top: r.top, h: r.height, pos: getComputedStyle(document.querySelector('.portal-topbar')).position }
    })
  )
  ...
  note('service page mobile: doc slots count + first slot buttons', {
    slots: await page.locator('.gov-doc-slot').count(),
    btns: await page.locator('.gov-doc-slot').first().locator('button').allTextContents(),
  })
  ...
  note(
    'national-id links',
    await page
      .locator('.official-handoff-links a')
      .evaluateAll(as => as.map(a => a.textContent.trim() + ' -> ' + a.href))
  )
