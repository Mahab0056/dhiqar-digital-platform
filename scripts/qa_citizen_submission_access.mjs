...
if (
  !genericGuestView.hasPublicShell ||
  genericGuestView.hasPortalSidebar ||
  !genericGuestView.hasForm ||
  !genericGuestView.notice.includes('الاستمارة متاحة')
) {
...
if (
  !specializedGuestView.hasPublicShell ||
  specializedGuestView.hasPortalSidebar ||
  !specializedGuestView.hasForm ||
  !specializedGuestView.notice.includes('الاستمارة متاحة') ||
  specializedGuestView.hasFakeCitizenName
) {
...
if (
  !verifiedFaceView.hasFaceSection ||
  !verifiedFaceView.hasFaceCapture ||
  !verifiedFaceView.hasConsent ||
  !verifiedFaceView.notice.includes('جاهز للإرسال')
) {
  throw new Error(`مطلب فيديو الوجه للحساب الموثق غير ظاهر: ${JSON.stringify(verifiedFaceView)}`)
}
await evaluate(
  `(() => { const form = document.querySelector('form.dynamic-service-form'); form.noValidate = true; form.requestSubmit(); return true })()`
)
await delay(150)
const faceValidationMessage = await evaluate(`document.querySelector('.form-error')?.textContent?.trim() || ''`)
if (!faceValidationMessage.includes('فيديو توثيق الوجه'))
  throw new Error(`لم تمنع الواجهة الإرسال من دون فيديو وجه: ${faceValidationMessage}`)

await go('/onboarding?continue=%2Fservice%2Fwater-complaint')
await delay(350)
const savedAccountResume = await evaluate(
  `(() => ({ path: location.pathname, restoredValues: [...document.querySelectorAll('[name]')].map(element => element.value) }))()`
)
if (
  savedAccountResume.path !== '/service/water-complaint' ||
  !savedAccountResume.restoredValues.includes('معلومة اختبار محفوظة')
) {
...
console.log(
  JSON.stringify(
    {
      citizen_submission_access: 'pass',
      genericGuestView,
      redirected: { path: redirected.path, continue: redirected.continue, draftSaved: Boolean(redirected.draft) },
      specializedGuestView,
      verifiedFaceView,
