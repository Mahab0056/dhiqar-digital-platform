...
socket.addEventListener('message', (event) => {
  ...
  if (message.method === 'Runtime.exceptionThrown') {
    ...
    notices.push({ type: 'exception', text: details.exception?.description || details.text, line: details.lineNumber, column: details.columnNumber });
  }
  if (message.method === 'Runtime.consoleAPICalled') {
    notices.push({ type: 'console', level: message.params.type, text: message.params.args.map((item) => item.value ?? item.description ?? '').join(' ') });
  }
  ...
  if (message.method === 'Network.responseReceived') {
    ...
    network.push({ type: message.params.type, status: response.status, mimeType: response.mimeType, url: response.url, fromDiskCache: response.fromDiskCache });
  }
  if (message.method === 'Network.loadingFailed') {
    network.push({ type: 'failure', error: message.params.errorText, blockedReason: message.params.blockedReason || null });
  }
});
