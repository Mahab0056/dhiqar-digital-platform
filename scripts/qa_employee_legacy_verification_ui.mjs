...
await send('Page.navigate', { url: `${baseUrl}/employee` })
await wait(650)
const initial = await evaluate(
  `(() => { const buttons = Array.from(document.querySelectorAll('button')); const approve = buttons.find(button => button.textContent?.includes('بانتظار فيديو الوجه')); return { hasReason: document.body.innerText.includes('لا يمكن اعتماد هذه المعاملة حالياً'), hasEmptyNotice: document.body.innerText.includes('معاملة سابقة لم تحفظ مرفقات التوثيق'), requestLabel: buttons.find(button => button.textContent?.includes('طلب استكمال التوثيق'))?.textContent?.trim() || '', approveDisabled: Boolean(approve?.disabled) } })()`
)
if (
  !initial.hasReason ||
  !initial.hasEmptyNotice ||
  !initial.approveDisabled ||
  !initial.requestLabel.includes('طلب استكمال التوثيق')
)
  throw new Error(`رسالة منع الاعتماد غير مكتملة: ${JSON.stringify(initial)}`)
await evaluate(
  `(() => { Array.from(document.querySelectorAll('button')).find(button => button.textContent?.includes('طلب استكمال التوثيق'))?.click(); return true })()`
)
await wait(250)
const afterRequest = await evaluate(
  `(() => ({ requestCount: window.__verificationRequestCount, text: document.body.innerText.includes('يرجى رفع فيديو توثيق الوجه القصير لإكمال التدقيق.') }))()`
)
if (afterRequest.requestCount !== 1 || !afterRequest.text)
  throw new Error(`طلب استكمال التوثيق لم ينفذ: ${JSON.stringify(afterRequest)}`)
console.log(JSON.stringify({ employee_legacy_verification_ui: 'pass', initial, afterRequest }, null, 2))
socket.close()
