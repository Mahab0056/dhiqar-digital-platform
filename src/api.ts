import type {
  ...
  GovernmentServiceDirectoryEntry,
...
const request = async <T>(path: string, options?: RequestInit): Promise<T> => {
  ...
  try {
    ...
    throw new Error(
      'تعذر الاتصال بالمنصة. تحقق من اتصال الإنترنت ثم أعد الإرسال؛ لم يُسجل الطلب ما لم يظهر رقم متابعة.'
    )
  }
  if (!response.ok) {
...
export const api = {
  ...
  listGovernmentServices: (params: { query?: string; dhiQarOnly?: boolean } = {}) => {
    ...
    if (params.dhiQarOnly) search.set('dhiQar', 'true')
    return request<GovernmentServiceDirectoryEntry[]>(`/api/government-services${search.size ? `?${search}` : ''}`)
  },
  getGovernmentService: (id: string) =>
    request<GovernmentServiceDirectoryEntry>(`/api/government-services/${encodeURIComponent(id)}`),
  listGovernmentServicesForAdmin: (status?: GovernmentServicePublicationStatus) =>
    request<{
      services: GovernmentServiceDirectoryEntry[]
      stats: { total: number; approved: number; needsReview: number; verified: number }
    }>(`/api/super-admin/government-services${status ? `?status=${status}` : ''}`),
  setGovernmentServicePublication: (
    id: string,
    publicationStatus: GovernmentServicePublicationStatus,
    reason?: string
  ) =>
    request<GovernmentServiceDirectoryEntry>(
  ...
  listSuperAdminCitizens: (filters: { query?: string; verificationStatus?: string; documentType?: string } = {}) => {
    ...
    return request<{ citizens: AdminCitizenDirectoryItem[] }>(
      `/api/super-admin/citizens${params.size ? `?${params.toString()}` : ''}`
    )
  },
  ...
  createServiceRequestWithFace: async (
    ...
    if (!response.ok) {
      ...
      throw new Error(readableRequestError(response.status, body.message))
    }
  ...
  uploadServiceRequestDocument: async (reference: string, documentName: string, document: File) => {
    ...
    if (!response.ok) {
      const body = await response.json().catch(() => ({ message: 'تعذر رفع المستند المطلوب.' }))
      throw new Error(body.message || 'تعذر رفع المستند المطلوب.')
    }
  ...
  createFeedback: async (payload: {
    ...
    if (!response.ok) {
      const body = await response.json().catch(() => ({ message: 'تعذر تسجيل الطلب.' }))
      throw new Error(body.message || 'تعذر تسجيل الطلب.')
    }
  ...
  loadFeedbackMedia: async (reference: string, mediaId: string) => {
    ...
    if (!response.ok) {
      const body = await response.json().catch(() => ({ message: 'تعذر فتح المرفق.' }))
      throw new Error(body.message || 'تعذر فتح المرفق.')
    }
  ...
  updateFeedback: (
    reference: string,
    payload: { status: Exclude<FeedbackStatus, 'RECEIVED'>; currentAction: string; adminNote?: string }
  ) => request<CitizenFeedback>(`/api/admin/feedback/${reference}`, { method: 'PATCH', body: JSON.stringify(payload) }),
  getNotifications: () => request<{ unread: number; items: CitizenNotification[] }>('/api/citizen/notifications'),
  markNotificationRead: (id: string) =>
    request<{ unread: number; items: CitizenNotification[] }>(`/api/citizen/notifications/${id}/read`, {
      method: 'PATCH',
    }),
  ...
  previewIdentityDocument: async (payload: {
    ...
    const response = await fetch('/api/onboarding/identity-extract-preview', {
      method: 'POST',
      body: form,
      credentials: 'include',
    })
    if (!response.ok) {
      const body = await response.json().catch(() => ({ message: 'تعذر تحليل صورة المستند.' }))
      throw new Error(body.message || 'تعذر تحليل صورة المستند.')
    }
    return response.json() as Promise<{
      status: string
  ...
  submitIdentityReview: async (payload: {
    ...
    if (payload.location) {
      form.append('locationLat', String(payload.location.lat))
      form.append('locationLng', String(payload.location.lng))
      if (payload.location.accuracyM !== undefined) form.append('locationAccuracyM', String(payload.location.accuracyM))
    }
    ...
    return response.json() as Promise<{
      ...
      files: Array<{ id: string; purpose: string; sizeBytes: number; retentionPolicy: string }>
      screening: {
  ...
  createApplicationWithFiles: async (
    ...
    let response: Response
    try {
      response = await fetch('/api/applications', { method: 'POST', body: form, credentials: 'include' })
    } catch {
      throw new Error('تعذر الاتصال بالمنصة. تحقق من الإنترنت ثم أعد الإرسال؛ لم تُسجل المعاملة ما لم يظهر رقم متابعة.')
    }
    if (!response.ok) {
  ...
  uploadMissingDocument: async (
    reference: string,
    documentName: string,
    document: File,
    documentPurpose: 'APPLICATION_DOCUMENT' | 'FACE_VIDEO' = 'APPLICATION_DOCUMENT'
  ) => {
    const form = new FormData()
    form.append('documentName', documentName)
    form.append('documentPurpose', documentPurpose)
    form.append('document', document)
    const response = await fetch(`/api/applications/${reference}/upload-document`, {
      method: 'POST',
      body: form,
      credentials: 'include',
    })
    if (!response.ok) {
      const body = await response.json().catch(() => ({ message: 'تعذر رفع المستند.' }))
      throw new Error(body.message || 'تعذر رفع المستند.')
    }
    return response.json() as Promise<GovernmentApplication>
  },
