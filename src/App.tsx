...
import type { Citizen, CitizenFeedback, CitizenNotification, CitizenServiceRequest, DashboardStats, GovernmentApplication } from './types'
import './App.css'
...
function LandingPage() {
  ...
  const quickActions = [
    ...
    { icon: CalendarDays, title: 'احجز موعداً', href: '/service/online-appointment' },
  ...
  return <div className="public-shell"><PublicHeader /><main>
    <section className="hero hero-modern"><div className="hero-ambient hero-ambient-one" /><div className="hero-ambient hero-ambient-two" /><div className="container hero-grid"><motion.div className="hero-copy" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.48, ease: [0.23, 1, 0.32, 1] }}><div className="eyebrow"><Landmark size={16} /> البوابة الرسمية للخدمات الرقمية</div><h1>ذي قار،<br /><em>أقرب إليك.</em></h1><p><strong>كيف نكدر نخدمك اليوم؟</strong> ابدأ معاملتك، تابع طلبك، احجز موعداً أو قدّم بلاغك من مكان واحد بخطوات واضحة ومهيأة للهاتف.</p><form className="smart-search smart-search-live" onSubmit={submitServiceSearch}><div className="search-icon"><Search size={21} /></div><label><small>ابحث باسم الخدمة أو حاجتك</small><input value={serviceQuery} onChange={event => setServiceQuery(event.target.value)} placeholder="مثال: أريد إجازة محل، أتابع معاملتي، عندي مشكلة ماء" aria-label="البحث عن خدمة" /></label><button type="submit" className="search-submit" aria-label="فتح أول نتيجة" disabled={!matchedServices.length}><ArrowLeft /></button>{serviceQuery && <div className="hero-search-results">{matchedServices.length ? matchedServices.map(service => <Link href={`/service/${service.key}`} key={service.key} onClick={() => setServiceQuery('')}><span><FileText /></span><div><strong>{service.title}</strong><small>{service.department}</small></div><ArrowLeft /></Link>) : <p>لا توجد خدمة مطابقة. جرّب كلمة أخرى.</p>}</div>}</form><div className="hero-actions"><Link href="/#services" className="button primary">ابدأ خدمة <ArrowLeft /></Link><Link href="/citizen" className="button hero-secondary">تابع معاملتك</Link></div><div className="hero-trust"><span><ShieldCheck size={17} /> تشفير للمرفقات الحساسة</span><span><Fingerprint size={17} /> تحقق متعدد المراحل</span><span><Eye size={17} /> تتبّع وسجل إجراءات</span></div></motion.div><motion.div className="hero-visual hero-visual-modern" initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.72, ease: [0.23, 1, 0.32, 1] }}><img src="/brand/landmarks/ur-ziggurat-panorama.jpg" alt="زقورة أور في محافظة ذي قار" /><div className="heritage-caption"><span>من أور إلى حكومة رقمية حديثة</span><small>صورة مرخصة للملكية العامة — Wikimedia Commons</small></div><div className="hero-command-card"><div className="command-head"><span className="live-badge"><i /> الخدمة متاحة</span><small>تحديث مباشر من سجل المنصة</small></div><div className="command-grid"><span><b>{stats.todayApplications.toLocaleString('en-US')}</b><small>معاملة اليوم</small></span><span><b>{stats.completed.toLocaleString('en-US')}</b><small>معاملة مكتملة</small></span><span><b>{(stats.registry?.verified ?? stats.departments.length).toLocaleString('en-US')}</b><small>جهة موثقة المصدر</small></span></div><div className="command-route"><MapPin /><span>ذي قار</span><i /><span>الخدمة المختصة</span><i /><span>المواطن</span></div></div><div className="civilization-line">إرث راسخ <ArrowLeft size={15} /> خدمة حديثة</div></motion.div></div></section>
    <NewsCarousel />
...
function LoginPage() {
  const options = [
    ...
    { icon: MonitorCheck, title: 'غرفة العمليات', text: 'وصول مقيّد للإدارة التشغيلية', href: '/operations/login', tone: 'operations' },
    { icon: ShieldCheck, title: 'مدير النظام', text: 'إدارة المنصة وسجل التدقيق', href: '/super-admin/login', tone: 'super-admin' },
  ]
  return <div className="login-page"><CivicUtilityBar /><div className="login-backdrop" /><div className="login-top container"><Brand /><Link href="/"><ArrowRight /> العودة للرئيسية</Link></div><main className="container login-content"><div className="login-intro"><span className="eyebrow"><LockKeyhole size={16} /> وصول محكوم حسب الدور</span><h1>بوابتك إلى ذي قار الرقمية</h1><p>اختر المسار الذي يخصك. تبقى حسابات المواطنين والجهات الحكومية مفصولة، ولكل منها صلاحياته وإجراءاته الموثقة.</p></div><div className="login-options">{options.map(option => <Link href={option.href} className={`login-option ${option.tone}`} key={option.title}><span><option.icon /></span><div><h2>{option.title}</h2><p>{option.text}</p></div><ArrowLeft /></Link>)}</div><div className="security-note"><ShieldCheck /><div><strong>بنية ثقة صفرية</strong><span>الدخول وحده لا يمنح الوصول؛ كل إجراء حساس يحتاج صلاحية وغرضاً مسجلاً.</span></div></div></main></div>
}
