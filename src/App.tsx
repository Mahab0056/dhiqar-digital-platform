...
function CitizenDashboard() {
  ...
  return <PortalLayout><section className="dashboard-welcome"><div><span className="verified-chip"><BadgeCheck /> {citizen?.verificationStatus === 'VERIFIED' || citizen?.verificationStatus === 'VERIFIED_MANUAL' ? 'هوية مراجعة ومعتمدة' : 'الهوية قيد المراجعة'}</span><h1>هلا {citizen?.fullName?.split(' ')[0] || 'مهاب'}، شنو تحتاج اليوم؟</h1><p>ابدأ خدمة جديدة أو تابع ما يحتاج تدخلك الآن.</p></div><Link href="/service/store-license" className="button primary"><Plus /> ابدأ معاملة</Link></section><div className="ai-service-search"><span><Sparkles /></span><div><small>مساعد الخدمات الذكي</small><strong>اكتب حاجتك باللهجة العراقية أو العربية</strong></div><button>اسأل الآن <ArrowLeft /></button></div><section className="citizen-summary-grid"><div className="summary-card attention"><span><Bell /></span><div><small>المطلوب منك الآن</small><strong>{applications.some(app => app.status === 'ACTION_REQUIRED') ? 'يرجى رفع مستند ناقص' : 'لا يوجد إجراء مطلوب منك'}</strong></div></div><div className="summary-card"><span><FileText /></span><div><small>المعاملات الجارية</small><strong>{applications.filter(a => a.status !== 'APPROVED').length}</strong></div></div><div className="summary-card"><span><FileCheck2 /></span><div><small>الوثائق الصادرة</small><strong>{applications.filter(a => a.status === 'APPROVED').length}</strong></div></div><div className="summary-card"><span><WalletCards /></span><div><small>المدفوعات</small><strong>{formatIQD(applications.reduce((s, a) => s + (a.paymentStatus === 'PAID' ? a.fee : 0), 0))}</strong></div></div></section><section className="dashboard-section citizen-notifications" id="notifications"><div className="dashboard-section-title"><div><h2>الإشعارات</h2><p>{unreadNotifications > 0 ? `${unreadNotifications.toLocaleString('en-US')} إشعار غير مقروء` : 'أنت مطّلع على جميع التحديثات.'}</p></div>{unreadNotifications > 0 && <button className="button outline" onClick={() => void readAllNotifications()}>تعليم الكل كمقروء</button>}</div><div className="notification-list">{notifications.length === 0 ? <div className="empty-state compact"><Bell /><h3>لا توجد إشعارات جديدة</h3><p>ستظهر هنا تحديثات الهوية والمعاملات والمستندات والمواعيد.</p></div> : notifications.map(item => item.link ? <Link href={item.link} className={item.readAt ? 'notification-item read' : 'notification-item unread'} key={item.id} onClick={() => { if (!item.readAt) void readNotification(item.id) }}><span><Bell /></span><div><strong>{item.title}</strong><p>{item.message}</p><time>{new Date(item.createdAt).toLocaleString('en-GB')}</time></div><ChevronLeft /></Link> : <button className={item.readAt ? 'notification-item read' : 'notification-item unread'} key={item.id} onClick={() => void readNotification(item.id)}><span><Bell /></span><div><strong>{item.title}</strong><p>{item.message}</p><time>{new Date(item.createdAt).toLocaleString('en-GB')}</time></div><Check /></button>)}</div></section><section className="dashboard-section"><div className="dashboard-section-title"><div><h2>معاملاتي</h2><p>حالة واضحة وما يجب فعله الآن.</p></div><Link href="/service/store-license">خدمة جديدة <Plus /></Link></div><div className="application-list">{applications.length === 0 ? <div className="empty-state"><FileText /><h3>لا توجد معاملات بعد</h3><p>ابدأ خدمة إجازة محل لإرسال طلبك ومرفقاتك إلى الدائرة المختصة.</p><Link className="button primary" href="/service/store-license">ابدأ إجازة محل</Link></div> : applications.map(app => <Link href={`/citizen/application/${app.reference}`} className="application-row" key={app.reference}><div className="application-icon"><BriefcaseBusiness /></div><div className="application-main"><div><strong>{app.serviceName}</strong><span className={`status ${app.status.toLowerCase()}`}>{statusLabels[app.status]}</span></div><small>{app.reference} • {app.department}</small><p>{app.currentAction}</p></div><ChevronLeft /></Link>)}</div></section><section className="dashboard-section"><div className="dashboard-section-title"><div><h2>طلبات الخدمات والمواعيد</h2><p>الاستمارات العامة وطلبات الحجز المسجلة في حسابك.</p></div><Link href="/service/online-appointment">حجز موعد <CalendarDays /></Link></div><div className="service-request-list">{serviceRequests.length === 0 ? <div className="empty-state compact"><CalendarDays /><h3>لا توجد طلبات عامة بعد</h3><p>اختر خدمة أو أرسل طلب موعد وسيظهر هنا رقم المتابعة والحالة.</p></div> : serviceRequests.map(request => { const definition = getServiceDefinition(request.serviceKey); return <article className="service-request-row" key={request.reference}><span className={request.appointment ? 'appointment' : 'service'}>{request.appointment ? <CalendarDays /> : <FileText />}</span><div><div><strong>{definition?.title || request.serviceKey}</strong><em>{request.status === 'APPOINTMENT_REQUESTED' ? 'بانتظار تأكيد الموعد' : 'تم التقديم'}</em></div><small>{request.reference} • {definition?.department || request.departmentId}</small><p>{request.currentAction}</p>{request.appointment && <time>{new Date(`${request.appointment.preferredDate}T00:00:00`).toLocaleDateString('en-GB')} • {request.appointment.preferredTime}</time>}</div></article> })}</div></section><section className="dashboard-section"><div className="dashboard-section-title"><div><h2>خدمات سريعة</h2><p>الأكثر استخداماً في منطقتك.</p></div></div><div className="portal-service-grid">{services.map(service => <Link href={`/service/${service.key}`} key={service.key}><span><BriefcaseBusiness /></span><strong>{service.title}</strong><small>{service.department}</small><ArrowLeft /></Link>)}</div></section></PortalLayout>
}
...
function SessionGate({ role, children }: { role: 'CITIZEN' | 'EMPLOYEE'; children: React.ReactNode }) {
  const [state, setState] = useState<'loading' | 'allowed' | 'denied'>('loading')
  useEffect(() => { api.getSession().then(session => setState(session.role === role || (role === 'EMPLOYEE' && session.role === 'IDENTITY_REVIEWER') ? 'allowed' : 'denied')).catch(() => setState('denied')) }, [role])
  if (state === 'loading') return <div className="access-gate-page"><RefreshCw className="spin" /><span>جاري التحقق من الجلسة...</span></div>
  if (state === 'denied') return <div className="access-gate-page denied"><Brand /><span className="access-gate-icon"><LockKeyhole /></span><h1>الدخول مطلوب</h1><p>{role === 'CITIZEN' ? 'أكد رقم هاتفك لإدارة معاملاتك وبياناتك بأمان.' : 'سجّل دخولك بحساب الموظف قبل فتح الشاشات التشغيلية.'}</p><Link className="button primary" href={role === 'CITIZEN' ? '/onboarding' : '/employee'}>{role === 'CITIZEN' ? 'تأكيد الهاتف' : 'دخول الموظف'} <ArrowLeft /></Link></div>
  return <>{children}</>
}
