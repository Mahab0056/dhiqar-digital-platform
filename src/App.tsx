...
function ApplicationPage({ reference }: { reference: string }) {
  ...
  const upload = async () => { if (!app) return; setBusy(true); await api.uploadMissingDocument(app.reference, app.requiredDocument || 'المستند المطلوب'); await refresh(); setBusy(false) }
...
function DhiQarMap({ departments }: { departments: DashboardStats['departments'] }) {
  const [selected, setSelected] = useState(departments[0]); const bounds = { minLat: 30.82, maxLat: 31.78, minLng: 46.02, maxLng: 46.55 }
  return <div className="dhiqar-map"><div className="map-topography" /><div className="dhiqar-outline" />{departments.map(dept => { const left = ((dept.lng - bounds.minLng) / (bounds.maxLng - bounds.minLng)) * 82 + 9; const top = (1 - (dept.lat - bounds.minLat) / (bounds.maxLat - bounds.minLat)) * 78 + 10; return <button key={dept.id} className={`department-marker ${dept.status.toLowerCase()} ${selected?.id === dept.id ? 'selected' : ''}`} style={{ left: `${left}%`, top: `${top}%` }} onClick={() => setSelected(dept)}><span><Building2 /></span><small>{dept.district}</small></button> })}{selected && <div className="map-detail-card"><div><span className={`system-status ${selected.status.toLowerCase()}`} /><small>{selected.status}</small></div><h3>{selected.name}</h3><p><MapPin /> {selected.district}، ذي قار</p><div><span><small>المعاملات</small><strong>{selected.transactions.toLocaleString('ar-IQ')}</strong></span><span><small>الأتمتة</small><strong>{selected.automation}%</strong></span></div><button>فتح التوأم الرقمي <ArrowLeft /></button></div>}<span className="map-caption">خريطة تشغيلية تجريبية — الإحداثيات تقريبية للعرض ويجب اعتماد سجلات GIS الرسمية قبل النشر</span></div>
}
...
function VerifyPage({ verificationId }: { verificationId: string }) {
  ...
  return <div className="verify-page"><DemoRibbon /><header className="verify-header container"><Brand /><Link href="/"><ArrowRight /> الرئيسية</Link></header><main className="container verify-content">{app ? <div className="verification-result valid"><span className="verification-icon"><BadgeCheck /></span><span className="section-kicker">OFFICIAL VERIFICATION PAGE — DEMO</span><h1>الوثيقة صحيحة ضمن بيئة العرض</h1><p>تم إصدار هذه الوثيقة من سيناريو ذي قار الرقمية التجريبي، وهي غير صالحة للاستخدام الرسمي.</p><div className="verification-data"><span><small>نوع الوثيقة</small><strong>إجازة ممارسة نشاط تجاري</strong></span><span><small>رقم الوثيقة</small><strong>{app.documentNumber}</strong></span><span><small>رقم المعاملة</small><strong>{app.reference}</strong></span><span><small>صاحب الوثيقة</small><strong>{app.citizenName}</strong></span><span><small>الحالة</small><strong>سارية — DEMO</strong></span><span><small>تاريخ الإصدار</small><strong>{new Date(app.updatedAt).toLocaleDateString('ar-IQ')}</strong></span></div><div className="verification-hash"><QrCode /><span><small>Verification ID</small><strong>{app.verificationId}</strong></span></div></div> : error ? <div className="verification-result invalid"><span className="verification-icon"><AlertTriangle /></span><h1>تعذر التحقق</h1><p>{error}</p><Link className="button primary" href="/">العودة للرئيسية</Link></div> : <div className="loading-state"><RefreshCw className="spin" /> جاري التحقق من الوثيقة...</div>}</main></div>
}
