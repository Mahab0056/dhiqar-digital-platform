...
function LandingPage() {
  ...
  return <div className="public-shell"><PublicHeader /><main>
    ...
    <section className="section government-strip"><div className="container government-grid"><div><span className="section-kicker">للحكومة المحلية</span><h2>صورة تشغيلية واحدة للمحافظة</h2><p>GIS، أداء الدوائر، SLA، الشكاوى، التحصيل المالي، صحة الأنظمة، وسجل تدقيق غير قابل للحذف من واجهات المستخدم.</p><Link href="/operations" className="button glass">افتح غرفة العمليات <ArrowLeft /></Link></div><div className="gov-metrics"><span><Activity /><b>99.96%</b><small>جاهزية الأنظمة</small></span><span><Gauge /><b>78%</b><small>مستوى الأتمتة</small></span><span><CircleDollarSign /><b>128.7م</b><small>تحصيل اليوم د.ع</small></span><span><Network /><b>31/33</b><small>دوائر متصلة</small></span></div></div></section><NewsAndProcurement />
  </main><Footer /></div>
}

function NewsAndProcurement() {
  return <><section className="section news-section"><div className="container"><div className="section-heading"><div><span className="section-kicker">أخبار ذي قار</span><h2>متابعة مصورة من المصادر المعروضة</h2></div><p>تحتفظ كل بطاقة برابط مصدرها. راجع المصدر للخبر الكامل وتاريخ النشر قبل اتخاذ أي إجراء.</p></div><div className="news-grid">{dhiqarNews.map((item, index) => <article className={index === 0 ? 'news-card featured' : 'news-card'} key={item.title}><img src={item.image} alt={item.title} loading="lazy" /><div><span>{item.category}</span><h3>{item.title}</h3><p>المصدر: {item.source}</p><a href={item.sourceUrl} target="_blank" rel="noreferrer">فتح المصدر <ArrowLeft /></a></div></article>)}</div></div></section><section className="section procurement-section"><div className="container"><div className="section-heading light"><div><span className="section-kicker">المناقصات والمزادات</span><h2>بوابة شفافة إلى الإعلانات المنشورة</h2></div><p>تظهر الروابط الرسمية المتاحة حالياً. لا تُنشأ أي مناقصة أو مزاد داخل المنصة قبل إدخال الإعلان من الجهة صاحبة الصلاحية.</p></div><div className="procurement-grid">{officialProcurementLinks.map(item => <a href={item.href} target="_blank" rel="noreferrer" key={item.title}><span><FileArchive /></span><div><small>{item.source}</small><h3>{item.title}</h3><p>{item.detail}</p></div><ArrowLeft /></a>)}</div></div></section></>
}
...
function EmployeeDashboard() {
  const [apps, setApps] = useState<GovernmentApplication[]>([]); const [selected, setSelected] = useState<GovernmentApplication | null>(null); const [busy, setBusy] = useState(false); const [reviewAccessCode, setReviewAccessCode] = useState(''); const [openedMedia, setOpenedMedia] = useState<{ url: string; mimeType: string; label: string } | null>(null); const [mediaError, setMediaError] = useState('')
...
function OperationsShell({ children, active = 'operations' }: { children: React.ReactNode; active?: string }) {
  return <div className="ops-shell"><DemoRibbon /><aside className="ops-sidebar"><Brand compact /><nav><Link href="/operations" className={active === 'operations' ? 'active' : ''}><Map /><span>غرفة العمليات</span></Link><Link href="/governor" className={active === 'governor' ? 'active' : ''}><Landmark /><span>لوحة المحافظ</span></Link><a href="/operations#departments"><Building2 /><span>الدوائر</span></a><a href="/operations#finance"><CircleDollarSign /><span>المالية</span></a><a href="/operations#operations-alerts"><MessageSquareWarning /><span>التنبيهات</span></a><a href="/operations#system-health"><Activity /><span>صحة النظام</span></a><Link href="/employee"><FileArchive /><span>التدقيق</span></Link></nav><Link href="/login" className="ops-exit"><LogIn /></Link></aside><main className="ops-main">{children}</main></div>
}
